  ...
  return (
    <svg width="300" height="220" viewBox="0 0 300 220" className="mx-auto">
      {[-4,-3,-2,-1,1,2,3,4].map(x => (
        <line key={x} x1={ox+x*s} y1={20} x2={ox+x*s} y2={200} stroke="var(--border)" strokeWidth="1"/>
      ))}
      {[-3,-2,-1,1,2,3,4].map(y => (
        <line key={y} x1={20} y1={oy-y*s} x2={280} y2={oy-y*s} stroke="var(--border)" strokeWidth="1"/>
      ))}
...
const StepsTable = () => (
  <svg width="300" height="185" viewBox="0 0 300 185" className="mx-auto">
    <rect x="5" y="5" width="290" height="175" rx="8" fill="var(--card)" stroke="#f59e0b" strokeWidth="1" strokeOpacity="0.4"/>
...
const ValueTable = (props: {func?: string, xs?: number[], ys?: (number|string)[], color?: string}) => {
  ...
  return (
    <svg width="300" height="110" viewBox="0 0 300 110" className="mx-auto">
      <rect x="5" y="5" width="290" height="100" rx="8" fill="var(--card)" stroke={color} strokeWidth="1" strokeOpacity="0.4"/>
      ...
      {['f(x)',...ys.map(String)].map((v,i)=>(
        <text key={i} x={22+i*37} y={68} fill={i===0?"#fbbf24":v==='...'?"#f472b6":v==='0'?"#86efac":v.toString().includes('-')?"#f472b6":"var(--card-foreground)"} fontSize="9" textAnchor="middle">{v}</text>
      ))}
...
const questions: Q[] = [
  ...
  Qn(9,"Identifikasi Grafik dari Ciri – TKA",{type:"mixed",diagram:(
    <svg width="300" height="200" viewBox="0 0 300 200" className="mx-auto">
      {[0,1,2,3].map(i => {
        ...
        return (
          <g key={i}>
            <rect x={cx-55} y={cy-44} width="110" height="88" rx="6" fill="var(--card)" stroke="#334155" strokeWidth="1"/>
  ...
  Qn(21,"Grafik Berdasarkan Tanda a dan c – TKA",{type:"mixed",diagram:(
    <svg width="300" height="140" viewBox="0 0 300 140" className="mx-auto">
      <rect x="5" y="5" width="290" height="130" rx="8" fill="var(--card)" stroke="#f59e0b" strokeWidth="1" strokeOpacity="0.4"/>
  ...
  Qn(25,"Soal UN – Pasangkan Grafik dengan Persamaan",{type:"mixed",diagram:(
    <svg width="300" height="200" viewBox="0 0 300 200" className="mx-auto">
      ...
      {[{cx:70,cy:70,up:true,c:"#f59e0b",l:"I"},{cx:220,cy:70,up:false,c:"#86efac",l:"II"},{cx:70,cy:155,up:true,c:"#f472b6",l:"III"},{cx:220,cy:155,up:false,c:"#a78bfa",l:"IV"}].map(g=>(
        <g key={g.l}>
          <rect x={g.cx-48} y={g.cy-42} width="96" height="84" rx="5" fill="var(--card)" stroke="#334155" strokeWidth="1"/>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={r.name} className={`${isDark ? "bg-white/5" : "bg-gray-50"} rounded-lg px-3 py-2`}>
                <div className={`${isDark ? "text-white/40" : "text-muted-foreground"} text-[9px] uppercase mb-1`}>{r.name}</div>
        ...
        <div className="flex flex-col gap-4 animate-slide-up">
          {questions.map((q,i)=>(
            <div key={q.n} className="relative rounded-2xl overflow-hidden animate-slide-up" style={{animationDelay:`${i*0.02}s`}}>
              <div className={`absolute inset-0 bg-gradient-to-br ${isDark ? "from-sky-900/30 via-slate-900/80 to-blue-900/30" : "from-sky-50/60 via-white/80 to-blue-50/40"} backdrop-blur`}/>
              ...
              <div className="relative px-5 py-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {q.mathContent && <div className={`mb-3 ${isDark ? "bg-sky-900/20" : "bg-sky-50"} border border-sky-500/20 rounded-lg px-4 py-3 flex justify-center`}><BlockMath math={q.mathContent}/></div>}
